...
\usepackage{xcolor}
\definecolor{cvblue}{HTML}{10635C}
\usepackage{titlesec}
\titleformat{\section}{\color{cvblue}\large\bfseries}{}{0pt}{}[\vspace{-6pt}\rule{\linewidth}{0.6pt}]
...
\begin{document}
""")
    A(r"{\Huge\bfseries %s}\par\vspace{4pt}" % tex_escape(S(prof.get("name"))))
    A(r"{\large %s}\par" % tex_escape(S(prof.get("position"))))
    if S(prof.get("position_second")):
        A(r"{\large %s}\par" % tex_escape(S(prof.get("position_second"))))
    A(r"\vspace{2pt}{\color{black!65}%s}\par" % tex_escape(
        ", ".join(x for x in [S(prof.get("organisation")), S(prof.get("address_line"))] if x)))
    if S(prof.get("email")):
        A(r"{\color{black!65}\href{mailto:%s}{%s}}\par" % (S(prof.get("email")), tex_escape(S(prof.get("email")))))
    A(r"\vspace{6pt}")
